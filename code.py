...
def open_main_app(window, username):
    ...
    def handle_voice_command():
        command = listen_for_command().lower().strip()
        print(f"You said: {command}")

        parsed = parse_command_with_gpt(command)
        if not parsed:
            speak_and_ding("Sorry, I couldn’t understand that.")
            return
        ...
        if action == "set" and medicine and time_str:
            speak_and_ding(f"Setting reminder for {medicine} at {time_str}.")
            with open(meds_file, "a", newline="") as f:
                writer = csv.writer(f)
            ...
            refresh_list()
        elif action == "delete" and medicine:
            speak_and_ding(f"Deleting reminder for {medicine}.")
            rows = []
            with open(meds_file, newline="") as f:
                reader = csv.reader(f)
